...
function ExpenseCardReceiptIcon({ 
  receiptUrl, 
  category, 
  isPdfReceipt, 
  getReceiptData 
  ...
  const realUrl = receiptUrl ? getReceiptData(receiptUrl) : '';

  if (realUrl && !isPdfReceipt(receiptUrl || null) && !imgError) {
    return (
      <img 
        src={realUrl} 
        alt="" 
  ...
  if (receiptUrl && isPdfReceipt(receiptUrl)) {
    return (
      <div className="w-full h-full bg-rose-50 dark:bg-rose-950/20 text-rose-500 flex flex-col items-center justify-center gap-0.5 rounded-2xl">
        <FileText className="w-5 h-5" />
        <span className="text-[7px] font-black uppercase tracking-widest leading-none">PDF</span>
      </div>
    );
  }
...
export default function TripDetail() {
  ...
  useEffect(() => {
    const resolvedUrl = previewReceipt ? getReceiptData(previewReceipt) : '';
    const isUrlAlreadyResolved = resolvedUrl && (
      resolvedUrl.startsWith('http://') || 
      resolvedUrl.startsWith('https://') || 
      resolvedUrl.startsWith('data:')
    );

    if (previewReceipt && previewStoragePath && !isUrlAlreadyResolved) {
      let isCurrent = true;
      const fetchFreshUrl = async () => {
        ...
        try {
          ...
          if (isCurrent && freshUrl) {
            setPreviewReceipt(freshUrl);
          }
        } catch (err) {
  ...
  }, [previewStoragePath, previewReceipt]);
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      // We will sync small compressed images directly in Firestore to guarantee they work for all users,
      // as Firebase Storage requires manual setup.
      if (localBase64.length > 900000) {
        toast.error("File is too large. Please use a smaller image or PDF under 700KB.");
        setIsUploadingFile(false);
        return;
      }
      
      // Directly set the base64 URL to sync via Firestore
      setReceiptImage(localBase64);
      setIsUploadingFile(false);
                    ...
                    <div className="flex items-center gap-4">
                      {expense.receiptUrl ? (
                        <button
                          ...
                          className="w-12 h-12 rounded-2xl bg-orange-50 dark:bg-orange-950/30 text-orange-500 flex items-center justify-center font-bold text-xl border border-orange-100/50 dark:border-orange-900/20 group-hover:scale-110 hover:border-orange-200 dark:hover:border-orange-850 transition-all shadow-sm group/thumb relative shrink-0 overflow-hidden"
                          title={isPdfReceipt(expense.receiptUrl) ? "View PDF Receipt" : "View Image Receipt"}
                        >
                          <ExpenseCardReceiptIcon 
                            receiptUrl={expense.receiptUrl} 
                            category={expense.category} 
                            isPdfReceipt={isPdfReceipt} 
                            getReceiptData={getReceiptData} 
                          />
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/thumb:opacity-100 transition-opacity flex items-center justify-center text-white text-[8px] font-bold uppercase tracking-widest leading-none">
                            Open
                          </div>
                        </button>
                      ) : (
                        <div className="w-12 h-12 rounded-2xl bg-orange-50 dark:bg-orange-950/30 text-orange-500 flex items-center justify-center font-bold text-xl border border-orange-100/50 dark:border-orange-900/20 shrink-0">
                          <ExpenseCardReceiptIcon 
                            category={expense.category} 
                            isPdfReceipt={isPdfReceipt} 
                            getReceiptData={getReceiptData} 
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1">
                          <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800/80 px-2 py-1 rounded-md">
                            <UserAvatar 
                              uid={expense.payerId}
                              displayName={expense.payerName}
                              photoURL={members.find(m => m.uid === expense.payerId)?.photoURL}
                              className="w-4 h-4 text-[6px] font-bold"
                            />
                        ...
                        <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-850 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-white">
                          ...
                          >
                            {isPdfReceipt(receiptImage) || receiptFileName?.toLowerCase().includes('.pdf') ? (
                              ...
                              <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 border border-slate-200 dark:border-slate-750 bg-black group-hover/preview:scale-110 transition-transform flex items-center justify-center">
                                {getReceiptData(receiptImage) ? (
                                  <img src={getReceiptData(receiptImage)} alt="Receipt preview" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                ) : (
                  ...
                  {qrUrl && (
                    <div className="mt-4 pt-4 border-t border-orange-100 dark:border-orange-500/10 flex flex-col items-center">
                      <div className="bg-white p-3 rounded-2xl shadow-md border border-orange-100 max-w-[150px]">
                        <img src={qrUrl} alt="Trip QR Code" className="w-[126px] h-auto select-none" referrerPolicy="no-referrer" />
                      </div>
      ...
      <AnimatePresence>
        {previewReceipt && (
          <motion.div 
            ...
            onClick={() => { setPreviewReceipt(null); setPreviewStoragePath(null); }}
          >
            <motion.div
            ...
            >
              ...
              <div className="flex flex-col gap-4 w-full">
                <div className="flex justify-center w-full max-h-[50vh] min-h-[300px] overflow-auto rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20">
                  {isPreviewResolving ? (
                    <div className="flex flex-col items-center justify-center p-8 text-center w-full min-h-[300px]">
                      <Loader2 className="w-8 h-8 animate-spin text-orange-500 mb-2" />
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">Loading Receipt...</p>
                    </div>
                  ) : isPdfReceipt(previewReceipt) ? (
                  ...
                  ) : getReceiptData(previewReceipt) ? (
                    <img src={getReceiptData(previewReceipt)} alt="Receipt" className="max-w-full h-auto object-contain rounded-xl" referrerPolicy="no-referrer" />
                  ) : (
                    <div className="py-12 flex flex-col items-center justify-center text-slate-500">
                      <ImageIcon className="w-12 h-12 mb-2 text-slate-300 dark:text-slate-600" />
                      <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">Receipt image not loaded</p>
                    </div>
                  )}
                </div>
